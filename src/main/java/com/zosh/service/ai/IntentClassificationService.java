...
public class IntentClassificationService {
    ...
    private static final Pattern OFF_TOPIC_PATTERN = Pattern.compile(
            "\\b(tell me a joke|sing a song|weather|capital of|who is the president|write code|write poem)\\b",
            Pattern.CASE_INSENSITIVE);
    ...
    private static final Pattern CART_VIEW_PATTERN = Pattern.compile(
            "\\b(what's in my cart|show my cart|view cart|check cart|check my cart|open cart|my cart|cart items)\\b",
            Pattern.CASE_INSENSITIVE);
    ...
    public ChatIntent classify(String rawMessage, ChatSession session) {
        ...
        // 1. Pure Conversational Checks (Must check before any commerce tools!)
        ...
        // 2. Strip greeting prefix if mixed intent ("Hi, show me shirts" -> "show me shirts")
        ...
        // 8. Contextual Follow-up Detection
        // If the session has a previous search, short refinements like "blue", "under 1500", "size L", "cheaper"
        // continue the product search or variant lookup
        if (session != null && session.getLastSearchQuery() != null) {
            if (isShortRefinement(strippedLower)) {
                return ChatIntent.PRODUCT_SEARCH;
            }
        }

        // 9. Default Shopping Search
        return ChatIntent.PRODUCT_SEARCH;
    }
    ...
    public String stripGreetingPrefix(String text) {
        ...
        if (m.find()) {
            ...
            // Only strip if there is actual remaining text
            if (!rest.isEmpty()) {
                return rest;
            }
        }
    ...
    private boolean hasCommerceAction(String lower) {
        return lower.contains("show") || lower.contains("find") || lower.contains("buy") ||
               ...
               lower.contains("return") || lower.contains("recommend");
    }

    private boolean isShortRefinement(String lower) {
        // Words <= 4 tokens that look like filter refinements
        String[] tokens = lower.split("\\s+");
        if (tokens.length > 5) return false;

        // Budget expressions: "under 1500", "below 2000"
        if (lower.matches(".*\\b(under|below|less than|within|upto|up to|max)\\s*\\d+.*")) return true;

        // Size expressions: "size l", "xl", "m"
        if (lower.matches(".*\\b(xs|s|m|l|xl|xxl)\\b.*")) return true;

        // Colors: "blue", "black", "teal", "red"
        String[] colors = {"red", "blue", "green", "black", "white", "yellow", "pink", "teal", "navy", "grey", "gray", "brown"};
        for (String c : colors) {
            if (lower.contains(c)) return true;
        }

        // Comparative words: "cheaper", "more expensive", "more", "different"
        if (lower.contains("cheaper") || lower.contains("more") || lower.contains("another")) return true;
